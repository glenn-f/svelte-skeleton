<script>
  ...
  const handler = new DataHandler(data.saidas || [], { rowsPerPage: 10 })
        ...
        {#each $rows as row}
          <tr>
            ...
            <td class="flex flex-nowrap justify-center">
              <IconButton href={`/estoque/saidas/${row.id}`} icon="fa6-solid:eye" data-tooltip="Ver Detalhes" data-placement="left" />
            </td>
          </tr>
        {:else}
          <tr>
