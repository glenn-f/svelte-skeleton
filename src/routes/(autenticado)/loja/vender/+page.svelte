<script>
  ...
  import ModalRcbto from './ModalRcbto.svelte'
  ...
  const clientes = writable(data.clientes || [])
  ...
  function abrirModalRcbto() {
    modalStore.trigger({
      type: 'component',
      component: { ref: ModalRcbto, props: { modo: 'adicionar', store: form, formas, totalFinal } }
    })
  }
  function abrirModalCliente() {
    modalStore.trigger({
      type: 'component',
      component: { ref: ModalCliente, props: { store: form, clientes, formData: data.formClientes } }
    })
  }
  ...
  const mapFormasSaida = new Map([
    [PE_VENDA, 'Venda'],
    [PE_VENDA_COM_BUYBACK, 'Venda com Buyback']
  ])

  function mountEid(eid) {
    if (eid) {
      modalStore.trigger({
        type: 'component',
        component: { ref: ModalItemSaida, props: { modo: 'adicionar', store: form, produtos, colaboradores, labelTitle: labelItem, eid } }
      })
    }
  }
...
<div class="grid place-items-center">
  <div class="container grid gap-3 bg-surface-100-800-token p-2 rounded-b-md">
    <div class="flex items-center">
      <!-- Titulo da Página -->
      <h2 class="h2 text-center mr-3">Venda de Estoque</h2>
    </div>
    <div class="grid grid-cols-12 gap-2">
      <!-- Campos Gerais -->
      <div class="col-span-12 grid place-content-center">
        <InputSelectRadio label="Forma de Saída" bind:value={$form.tipo_pe} options={mapFormasSaida} error={$errors.tipo_pe} required />
      </div>
      ...
      {#if isVenda}
        {@const text = $form.participante_id ? $clientes.find((v) => v.id == $form.participante_id)?.nome : 'Selecione'}
        {@const iconRight = $form.participante_id ? 'fa6-solid:arrows-rotate' : 'fa6-solid:check'}
        {@const color = $form.participante_id ? 'variant-filled-success' : 'variant-filled'}
        <div class="xl:col-span-2 col-span-6">
          <Label label="Cliente" labelClass="grid">
            <Button class={color} on:click={abrirModalCliente} {text} {iconRight} />
          </Label>
        </div>
      {/if}
      <!--* Itens Saida -->
      ...
      <div class="col-span-12 flex gap-2 justify-center items-center">
        ...
        <div class="grid gap-2">
          ...
          <form action="?/vender" bind:this={inputForm} method="POST" use:enhance>
            <button
              type="submit"
