<script>
  ...
  function handleAdicionar() {
    modalStore.trigger({
      type: 'component',
      component: { ref: ModalFormComissao, props: { modo: 'adicionar', formData: data.formAdicionar } }
    })
  }
  function handleEditar(row) {
    modalStore.trigger({
      type: 'component',
      component: { ref: ModalFormComissao, props: { modo: 'editar', initialData: { ...row }, formData: data.formEditar } }
    })
  }
  ...
  <div class="container">
    <div class="grid gap-3 bg-surface-100-800-token p-2 rounded-md">
      <div class="flex items-center">
        <h1 class="h1 text-center mr-3">Regras de Comissão</h1>
        ...
        <table class="table table-compact table-hover text-center">
          <thead class="!bg-surface-300-600-token whitespace-nowrap">
            <tr class="!text-center">
              <TH orderBy="nome">Nome da Regra</TH>
              <TH orderBy="descricao">Descrição</TH>
              <TH orderBy={(row) => formatTaxa(row.taxa_fixa) + '%'}>Taxa</TH>
              <TH orderBy={(row) => formatMoeda(row.bonus_fixo)}>Bônus</TH>
            ...
            <tr>
              ...
              <THF filterBy={(row) => formatMoeda(row.bonus_fixo)} />
              <td class="w-0" />
              <td class="w-0" />
            </tr>
          </thead>
          <tbody>
            {#each $rows as row}
              <tr>
                ...
                <td>{formatMoeda(row.bonus_fixo)}</td>
