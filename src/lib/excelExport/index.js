...
export default async function exportExcel(table, filename, applyFilters = true) {
	...
	ws.columns = lastHeaderGroup.headers
		.filter(
			(h) => h.column.getIsVisible() && !(h.column.columnDef.meta)?.noExport
		)
		.map((header) => {
			return {
	...
	exportRows.forEach((row) => {
		...
		const exportCells = cells.filter(
			(cell) => !(cell.column.columnDef.meta)?.noExport
		);
