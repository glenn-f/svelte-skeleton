<script>
  ...
  function handleMask(text, selEnd) {
    ...
    text = text.replace(/[.,]/g, '').padStart(3, '0')
    const numJs = parseFloat(text.slice(0, -2) + '.' + text.slice(-2))
    const numBr = formatMoeda(numJs, 2)
  ...
  function onChangeValue(v) {
    if (!maskInput) return
    v = parseFloat(v)
    let mask = Number.isFinite(v) ? formatMoeda(v, 2) : ''
