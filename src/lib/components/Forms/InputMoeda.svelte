<script>
  ...
  function onBeforeInput(e) {
    const tamSelecao = e.target.selectionEnd - e.target.selectionStart
    const cursorChar = e.target.value?.at(e.target.selectionStart)
    const lastCharEntrada = e.data?.at(-1)
    const temVirgula = e.target.value?.at(-3) === ','
    //* Inserção
    if (typeof e.data == 'string' && e.data.length > 0) {
      ...
      const finalText = e.target.value?.substring(0,e.target.selectionStart) + e.data + e.target.value?.substring(e.target.selectionEnd)
      console.log({i: e.target.value, f: finalText})
      //* Caso de entrada de uma virgula ou ponto
      if (e.inputType == 'insertText' && tamSelecao <= 1) {
        if (lastCharEntrada === cursorChar && (cursorChar === ',' || cursorChar === '.')) {
    ...
    //* Deleção, virgulas e pontos
    else if (e.inputType == 'deleteContentBackward' && tamSelecao <= 1) {
      //backspace
      let posAnt = tamSelecao == 0 ? e.target.selectionStart - 1 : e.target.selectionStart
      let charAnt = e.target.value.charAt(posAnt)
      if (charAnt === ',' || charAnt === '.') {
        e.target.setSelectionRange(posAnt, posAnt)
        return e.preventDefault()
      }
    } else if (e.inputType == 'deleteContentForward' && tamSelecao <= 1) {
      //del
      let char = e.target.value.charAt(e.target.selectionStart)
      if (char === ',' || char === '.') {
        e.target.setSelectionRange(e.target.selectionStart + 1, e.target.selectionStart + 1)
        return e.preventDefault()
      }
